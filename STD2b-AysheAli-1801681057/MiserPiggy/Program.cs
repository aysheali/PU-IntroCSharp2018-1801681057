...
namespace MiserPiggy
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Take user input
            Console.WriteLine("Hey, piggies! Enter your name:");
            ...
            // Finally, compare the results and display a suitable massage.
            if (sumPiggyName1 > sumPiggyName2 && sumPiggyName1 > sumPiggyName3)
            {
                if (sumPiggyName2 > sumPiggyName3)
                {
                    Console.WriteLine($"The name-wealthiest pig is {piggyName1} ({sumPiggyName1}) followed by" + $"{piggyName3} ({sumPiggyName3}) and last is {piggyName2} ({sumPiggyName2}).");
                }
                else
                {
